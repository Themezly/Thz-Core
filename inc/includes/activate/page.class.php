...
class ThzCore_Activation_Page{
	...
	public function __construct( $endpoint, ThzCore_Activation_Option $option ) {
		...
		if( !ThzCore_Update_Theme_Helper::not_uses_cp() || ThzCore_Update_Theme_Helper::has_cp() ){
		    return;
        }
    ...
    private function show_alert(){
	    ...
	    if( ThzCore_Update_Theme_Helper::requires_cp() ) {
		    $this->load();
            // show alert for CP and stop
            new ThzCore_Admin_Notice( __( "There is a new Creatus theme version. Before being able to update please make sure you install and activate Creatus Pro Plugin than visit the updates page again.", "thz-core" ),'notice notice-success is-dismissible' );
        }
  	}
