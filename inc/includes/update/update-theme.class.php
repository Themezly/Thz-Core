...
class ThzCore_Update_Theme{
	...
	public function check_update( $value/*, $transient*/ ){
		...
		if ( isset( $value->checked[ $this->theme ] ) ) {
			...
			if ( $theme ) {
				...
				ThzCore_Update_Theme_Helper::unregister_update();
				
				if( ThzCore_Update_Theme_Helper::needs_cp( $theme['version'] ) && !ThzCore_Update_Theme_Helper::has_cp() ){
					unset( $value->response[ $this->theme ] );
					ThzCore_Update_Theme_Helper::register_update( $theme );
	...
	private function get_details( $current_version ){
		...
		if( ThzCore_Update_Theme_Helper::has_cp() ){
